...
$errors = [];
$data = [
    'matricule' => '', 'first_name' => '', 'last_name' => '', 'sexe' => 'M',
    'date_naissance' => '', 'email' => '', 'telephone' => '', 'faculty_id' => '',
    'program_id' => '', 'niveau' => '', 'annee_academique' => '', 'statut' => 'actif',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare('INSERT INTO students (matricule, first_name, last_name, sexe, date_naissance, email, telephone, faculty_id, program_id, niveau, annee_academique, statut)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([
...
?>
<h1 class="h3 mb-4">Ajouter un étudiant</h1>
...
<form method="post" action="">
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-3">
            ...
            <input type="date" name="date_naissance" class="form-control" value="<?= old($data, 'date_naissance') ?>">
        </div>
        <div class="col-md-6 mb-3">
        ...
        <div class="col-md-6 mb-3">
            ...
            <select name="program_id" id="program_id" class="form-select">
                ...
                <?php foreach ($programs as $p): ?>
                    <option value="<?= $p['id'] ?>" data-faculty="<?= $p['faculty_id'] ?>" <?= $data['program_id'] === (int) $p['id'] ? 'selected' : '' ?>><?= e($p['nom']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-6 mb-3">
            <label class="form-label">Niveau</label>
            <input type="text" name="niveau" class="form-control" value="<?= old($data, 'niveau') ?>" placeholder="ex: Licence 4">
        </div>
        <div class="col-md-6 mb-3">
